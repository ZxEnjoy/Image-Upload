...
const allfun = (image, param, cut_param) => {
  ...
  if (param[0] && param[1]) {
    ...
    ctx.putImageData(imgData, 0, 0);
    return webCanvas.transferToImageBitmap();
    // if (hasBlob) {
...
onmessage = (param) => {
  postMessage({
    imgdata: allfun(param.data.imgdata, param.data.param, param.data.cutParam),
    name: param.data.name,
  });
  // ctx.drawImage(param.data.imgdata, 0, 0);
  // postMessage({
  //   imgdata: param.data.imgdata,
  //   some: { ...param }.toString(),
  // });
};
// export default () => {
